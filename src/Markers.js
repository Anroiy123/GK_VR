...
function buildPhotoUrls(keywords, lockBase) {
  const keywordPath = encodeURIComponent(keywords.join(',')).replaceAll('%2C', ',');
...
function shadeColor(hex, offset) {
  ...
  const hsl = {};
  color.getHSL(hsl);
  color.setHSL(hsl.h, Math.max(0, Math.min(1, hsl.s + offset * 0.15)), Math.max(0, Math.min(1, hsl.l + offset)));
  return color;
}

function createStandardMaterial(color, lightnessOffset = 0, emissiveBoost = 0.08) {
...
export class Markers {
  ...
  createMarkers() {
    LOCATIONS.forEach((location, index) => {
      ...
      const basePosition = this.latLonToVector3(location.lat, location.lon, this.earthRadius);
  ...
  createMountain(color) {
    ...
    const base = createMesh(new THREE.ConeGeometry(0.075, 0.18, 6), rockMaterial, 0, 0.09, 0);
    base.rotation.y = Math.PI / 6;
    const ridge = createMesh(new THREE.ConeGeometry(0.05, 0.14, 5), ridgeMaterial, 0.04, 0.09, -0.02);
    ridge.rotation.z = -0.18;
    const snow = createMesh(new THREE.ConeGeometry(0.028, 0.05, 5), snowMaterial, 0.012, 0.165, 0.002);

    group.add(base, ridge, snow);
    return { group, materials: [rockMaterial, ridgeMaterial, snowMaterial], height: 0.22 };
  }
  ...
  createTorii(color) {
    ...
    const leftPillar = createMesh(new THREE.BoxGeometry(0.022, 0.16, 0.022), pillarMaterial, -0.055, 0.08, 0);
    const rightPillar = createMesh(new THREE.BoxGeometry(0.022, 0.16, 0.022), pillarMaterial, 0.055, 0.08, 0);
    const topBeam = createMesh(new THREE.BoxGeometry(0.16, 0.024, 0.032), beamMaterial, 0, 0.165, 0);
    const midBeam = createMesh(new THREE.BoxGeometry(0.12, 0.018, 0.026), beamMaterial, 0, 0.13, 0);
    const tie = createMesh(new THREE.BoxGeometry(0.032, 0.05, 0.018), accentMaterial, 0, 0.11, 0);

    group.add(leftPillar, rightPillar, topBeam, midBeam, tie);
    return { group, materials: [pillarMaterial, beamMaterial, accentMaterial], height: 0.23 };
  }
  ...
  createSkyline(color) {
    ...
    const left = createMesh(new THREE.BoxGeometry(0.045, 0.13, 0.04), towerMaterial, -0.05, 0.065, 0);
    const center = createMesh(new THREE.BoxGeometry(0.05, 0.19, 0.04), glassMaterial, 0.005, 0.095, 0);
    const right = createMesh(new THREE.BoxGeometry(0.038, 0.11, 0.036), towerMaterial, 0.055, 0.055, 0);
    const torchStem = createMesh(new THREE.CylinderGeometry(0.008, 0.008, 0.08, 8), torchMaterial, -0.01, 0.17, 0.028);
    const torchFlame = createMesh(new THREE.SphereGeometry(0.018, 16, 16), torchMaterial, -0.01, 0.222, 0.028);
    torchFlame.scale.set(0.7, 1.15, 0.7);

    group.add(left, center, right, torchStem, torchFlame);
    return { group, materials: [towerMaterial, glassMaterial, torchMaterial], height: 0.26 };
  }
  ...
  createEiffelTower(color) {
    ...
    const lowerBeam = createMesh(new THREE.BoxGeometry(0.11, 0.018, 0.022), accentMaterial, 0, 0.09, 0);
    const midTower = createMesh(new THREE.BoxGeometry(0.038, 0.1, 0.028), frameMaterial, 0, 0.17, 0);
    const topTower = createMesh(new THREE.BoxGeometry(0.018, 0.07, 0.018), accentMaterial, 0, 0.245, 0);
    const spire = createMesh(new THREE.CylinderGeometry(0.004, 0.004, 0.05, 8), accentMaterial, 0, 0.305, 0);
  ...
  createPyramid(color) {
    ...
    const main = createMesh(new THREE.ConeGeometry(0.085, 0.16, 4), stoneMaterial, 0, 0.08, 0);
    main.rotation.y = Math.PI / 4;
    const side = createMesh(new THREE.ConeGeometry(0.048, 0.095, 4), shadowMaterial, 0.055, 0.05, 0.03);
  ...
  createTree(color) {
    ...
    const trunk = createMesh(new THREE.CylinderGeometry(0.014, 0.02, 0.12, 10), trunkMaterial, 0, 0.06, 0);
    const canopy = createMesh(new THREE.SphereGeometry(0.07, 18, 18), canopyMaterial, 0, 0.14, 0);
    canopy.scale.set(1.1, 0.9, 1.1);
    const canopyTop = createMesh(new THREE.SphereGeometry(0.045, 16, 16), canopyShadowMaterial, 0.02, 0.19, -0.01);

    group.add(trunk, canopy, canopyTop);
    return { group, materials: [trunkMaterial, canopyMaterial, canopyShadowMaterial], height: 0.25 };
  }
  ...
  createKhueVanCac(color) {
    ...
    const base = createMesh(new THREE.BoxGeometry(0.12, 0.04, 0.08), baseMaterial, 0, 0.02, 0);
    const pillarGeometry = new THREE.BoxGeometry(0.016, 0.09, 0.016);
    const leftFront = createMesh(pillarGeometry, frameMaterial, -0.036, 0.085, 0.022);
    const rightFront = createMesh(pillarGeometry, frameMaterial, 0.036, 0.085, 0.022);
    const leftBack = createMesh(pillarGeometry, frameMaterial, -0.036, 0.085, -0.022);
    const rightBack = createMesh(pillarGeometry, frameMaterial, 0.036, 0.085, -0.022);
    const beam = createMesh(new THREE.BoxGeometry(0.1, 0.02, 0.065), frameMaterial, 0, 0.135, 0);
    const roof = createMesh(new THREE.ConeGeometry(0.082, 0.07, 4), roofMaterial, 0, 0.18, 0);
    roof.rotation.y = Math.PI / 4;

    group.add(base, leftFront, rightFront, leftBack, rightBack, beam, roof);
    return { group, materials: [baseMaterial, frameMaterial, roofMaterial], height: 0.26 };
  }
  ...
  createVRPanel(location) {
    ...
    const background = createSpriteFromTexture(this.createPanelBackgroundTexture(), 1.16, 0.82);
    background.center.set(0.5, 0.5);
    panel.add(background);

    const info = createSpriteFromTexture(this.createInfoTexture(location.name, location.desc), 0.98, 0.3);
    info.position.set(0, 0.21, 0.02);
    panel.add(info);

    const imageOffsets = [-0.32, 0, 0.32];
    const imageSlots = imageOffsets.map((offsetX) => {
      const sprite = createSpriteFromTexture(this.loadingCardTexture, 0.28, 0.18);
  ...
  update(timestamp, camera) {
    ...
    this.markersList.forEach((marker) => {
      const sinWave = Math.sin(timestamp * 0.0025 + marker.userData.phaseOffset) * 0.006;
      const normal = marker.userData.basePosition.clone().normalize();
      marker.position.copy(marker.userData.basePosition.clone().add(normal.multiplyScalar(sinWave)));

      marker.getWorldPosition(this.worldMarkerPosition);
      marker.userData.surfaceNormal.copy(this.worldMarkerPosition).sub(this.worldEarthCenter).normalize();
      marker.userData.isFrontFacing = this.isVisible && marker.userData.surfaceNormal.dot(this.cameraDirection) > 0;
